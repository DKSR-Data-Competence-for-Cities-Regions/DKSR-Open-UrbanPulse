...
public class SetupMasterConnectorTest {
    ...
    public void test_addModuleSetup_saves_the_added_DAOs() {
        ...
        Mockito.doReturn(UPModuleType.Backchannel).when(backchannelSetupDAO).getModuleType();
        ...
        setupMasterConnector.addModuleSetup(backchannelSetupDAO);
    ...
    public void test_reset_calls_all_needed_methods() {
        ...
        Mockito.doNothing().when(backchannelSetupDAO).clearAssignments();
        ...
        Mockito.verify(backchannelSetupDAO).clearAssignments();
    ...
    @Test
    public void test_sendSetup_no_configs_in_use() {
        String moduleId = "1";
        String dummyStr = "test";
        Map<String, Object> args = new HashMap<>();
        args.put("id", moduleId);
        UPModuleEntity customModuleEntity = new UPModuleEntity();
        customModuleEntity.setId(moduleId);
        customModuleEntity.setModuleType(UPModuleType.Backchannel.name());
        JsonObject moduleSetupDummy = new JsonObject().put("dummy", dummyStr);
        
        BackchannelSetupEntity backChannelSetupEntity = new BackchannelSetupEntity();
        backChannelSetupEntity.setId(1L);
        backChannelSetupEntity.setModuleId(moduleId);
        backChannelSetupEntity.setSetupJson("{}");
        List<BackchannelSetupEntity> setupEntitiesList = new ArrayList();
        setupEntitiesList.add(backChannelSetupEntity);
        
        Mockito.doReturn(UPModuleType.Backchannel).when(backchannelSetupDAO).getModuleType();
        Mockito.doReturn(customModuleEntity).when(moduleDAO).queryById(moduleId);
        Mockito.doReturn(setupEntitiesList).when(backchannelSetupDAO).queryAll();
        Mockito.doReturn(moduleSetupDummy).when(backchannelSetupDAO).createModuleSetup(any(), any());
        Mockito.doNothing().when(heartbeatHandler).receivedHeartbeatForModule(anyString(), any());
        ...
        setupMasterConnector.sendSetup(args, true, (result, command) -> {
            assertFalse(result.isEmpty());
            assertEquals(dummyStr, result.getString("dummy"));
            Mockito.verify(backchannelSetupDAO, Mockito.times(0)).unassign(anyString());
        });
    }
